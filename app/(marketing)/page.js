        ...
        <Box className="flex flex-col gap-2">
          ...
          {domainSlugs.map((slug) => (
            <CtaButton
              ...
              render={<Link href={`/corporate-training/domain/${slug}`} />}
            >
              /corporate-training/domain/{slug}
            </CtaButton>
          ))}
        </Box>
        ...
        <Box className="flex flex-col gap-2">
          ...
          {vendorSlugs.map((slug) => (
            <CtaButton
              key={slug}
              className="self-start"
              render={<Link href={`/corporate-training/vendor/${slug}`} />}
            >
              /corporate-training/vendor/{slug}
            </CtaButton>
          ))}
        </Box>
      </Box>
