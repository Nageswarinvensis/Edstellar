...
export default function Benefits({ data }) {
  ...
  const prefixText = heading.prefix || "Why";
  const highlightText = heading.highlightText || "automate";
  const suffixText = heading.suffix || "your TNA?";
      ...
      <Box>
        ...
        <Reveal delay={1}>
          {subtitle && (
            <Text
              as="p"
              className="mt-3 text-[16px] text-ink/70"
            >
              {subtitle}
            </Text>
          )}
          ...
          <Box className="flex flex-col bg-[#EFECE6]/70">
            ...
            <Box className="flex-1 divide-y divide-black/5">
              {(traditional.items || []).map((item, idx) => (
                <Box key={idx} className="flex items-center gap-3.5 px-6 py-4">
