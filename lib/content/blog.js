 ...
 */
export const getBlogAuthor = cache(async (slug, page = 1) => {
  const payload = await apiGet(endpoints.blogAuthor(slug, page), {
    revalidate: REVALIDATE.author,
  });
  return payload?.author ?? null;
});
 ...
 */
export const getBlogCategory = cache(async (slug, page = 1) => {
  const payload = await apiGet(endpoints.blogCategory(slug, page), {
 ...
 */
export const getBlogMain = cache(async (page = 1) => {
  const payload = await apiGet(endpoints.blogMain(page), {
