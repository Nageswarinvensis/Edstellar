...
/**
 * Which of the shared stylesheets under components/sections/blog/detail/blog-content
...
const SANITIZE_OPTIONS = {
  ...
  allowedAttributes: {
    ...
    th: ["scope", "class", "width", "height", "colspan", "rowspan"],
    td: ["width", "height", "class", "colspan", "rowspan"],
  ...
  // Deliberately no `allowedStyles` here: sanitize-html only filters
  // `style="..."` values against an explicit per-property allowlist (see
  // `filterCss` in sanitize-html/index.js — no `allowedStyles` config at all
  // means every declaration passes through unfiltered). Raw table/list
  // embeds use an open-ended, growing set of inline styles for their own
  // layout (flex-wrap chip rows, column widths, etc.) that can't be
  // enumerated up front; curating individual properties meant a new embed
  // shape silently lost its layout until someone noticed and added the
  // missing property here. This is CMS body HTML we control (CLAUDE.md's
  // "no inline styles" rule is a Tailwind-authoring rule and has no lever
  // over it) — the risk is a stray layout-breaking value shipping verbatim,
  // not injection: `<style>` blocks are stripped and re-scoped separately by
  // `extractInlineStyle` above, and non-style/class/id attributes are still
  // governed by `allowedAttributes`.
