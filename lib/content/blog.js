...
/**
 * The bulk listing already carries every post's full `fieldData` — including
 * `content` — so a second per-slug request is not fetching anything the
 * listing didn't already return. Reusing it here is what took the build from
 * ~580 outbound requests (one per post, all fired during static generation)
 * down to the single bulk request `getBlogPosts` already makes, which is what
 * was tripping the blog API's rate limit in the first place.
 */
