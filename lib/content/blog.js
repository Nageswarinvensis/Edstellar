...
const API_BASE = "https://webflow-blog-api.vercel.app/api/blogs";

/**
 * Which of the shared stylesheets under app/blog/styles a post needs, keyed
...
function normalizePost(item) {
  ...
  return {
    ...
    seo: {
      title: fd["meta-title"] || fd.name,
      description: fd["meta-description"] || fd.description,
      canonical: fd["canonical-links"] || `/blog/${fd.slug}`,
      ogImage: fd["og-image"]?.url,
    },
...
export const getBlogPost = cache(async (slug) => {
  const item = await fetchJson(`${API_BASE}/${slug}`);
