...
const VENDOR_COURSES = {
  "ml-model-monitoring": {
    ...
    breadcrumbs: [
      ...
      { label: "Machine Learning", href: "/corporate-training/domain/machine-learning" },
    ...
    about: {
      ...
      contrast: {
        ...
        rows: [
          ["Watches uptime and errors", "Watches accuracy, drift, and fairness"],
          ["Fails loudly when it breaks", "Degrades silently as data shifts"],
          ["Fixed logic, stable over time", "Behavior decays without code changes"],
 ...
 */
export const getDomainCourse = cache(async (slug) => DOMAIN_COURSES[slug] ?? null);

export const getVendorCourse = cache(async (slug) => VENDOR_COURSES[slug] ?? null);

export const getDomainCourseSlugs = cache(async () => Object.keys(DOMAIN_COURSES));

export const getVendorCourseSlugs = cache(async () => Object.keys(VENDOR_COURSES));
