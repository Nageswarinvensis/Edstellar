 ...
 */
/**
 * Convert the CMS's `learning_journey` shape into the `method` shape the
 * `Curriculum` component reads. Called in `toCourse` when the CMS sends
 * `curriculum.learning_journey` — the result is merged as `curriculum.method`
 * so the rest of the pipeline never sees the CMS key.
 *
 * Mapping:
 *   steps[].title          → steps[].label
 *   steps[].description    → steps[].parts[0].text
 *   distribution[].label   → split[].label  (lowercased → split[].phase_key)
 *   distribution[].percentage → split[].percent
 *   highlight              → note  (plain string; <b> tags supported)
 *   delivered_as.items[].label → formats[]
 *   delivery_note          → summary_note
 */
function shapeLearningJourney(lj) {
  if (!lj) return null;

  const method = {};

  if (lj.steps?.length) {
    method.steps = lj.steps.map((step) => ({
      id: step.id,
      label: step.title,
  ...
  if (lj.distribution?.length) {
    // Use steps[i].id as phase_key (guaranteed to match the hardcoded color
    // maps) and steps[i].title as the display label, so typos in the
    // distribution labels don't break colors or legend text.
    method.split = lj.distribution.map((d, i) => ({
      phase_key: lj.steps?.[i]?.id ?? d.label.toLowerCase(),
      label: lj.steps?.[i]?.title ?? d.label,
      percent: d.percentage,
    }));
  }

  if (lj.highlight) method.note = lj.highlight;

  if (lj.delivered_as?.items?.length) {
    method.formats = lj.delivered_as.items
      .map((item) => ({ label: item.label, icon: item.icon }))
      .filter((item) => item.label);
    // Signal that the backend owns the full chip list — the Curriculum
    // component should skip its static META_PILL_FIELDS addition.
    method.formats_complete = true;
  }

  if (lj.delivery_note) method.summary_note = lj.delivery_note;
...
function toCourse(payload, domain) {
  ...
  // sends wins, whatever it omits falls back to the copy authored in
  // `COURSE_DEFAULTS`. `curriculum.method` has no default — it is
  // entirely CMS-driven.
  const { about: cmsAboutRaw, ...cmsRest } = fromCms;
  cmsRest.about = stripFixedAboutFields(cmsAboutRaw);

  // If the CMS sends `curriculum.learning_journey`, promote it to `method`
  // (the shape the Curriculum component reads) and drop the CMS key so it
  // never reaches JSX. Falls back to COURSE_DEFAULTS.curriculum.method when
  // the CMS sends neither field.
  if (cmsRest.curriculum?.learning_journey) {
    const { learning_journey, ...curriculumRest } = cmsRest.curriculum;
    const method = shapeLearningJourney(learning_journey);
    cmsRest.curriculum = method ? { ...curriculumRest, method } : curriculumRest;
  }
